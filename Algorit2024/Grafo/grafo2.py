...
class Graph:
    ...
    def barridoPaises(self,paises):
        for pais in paises:
            ...
            for index, nodo in enumerate(self.elements):
                print(nodo['other_value'])
                if pais in nodo['other_value']:
                    con+=1
                print()
        if con>1:
            print("el pais ",pais,"dispone de esta cantidad de maravillas: ",con)
        print("----------------")
    ...
    def search222(self, value):
        for index, element in enumerate(self.elements):
            if element['value'] == value:
                return element["other_value"]
            
    # def buscomaravillas(self, value):
    #     conM=0
    #     conN=0
    #     for index, element in enumerate(self.elements):
    #         other_valuestr=str(element['other_value'])
    #         if value in other_valuestr:
    #             con=con+1

    #     if con>1:
    #         print(value ,"de esta cantidad de maravillas",con)
    ...
    def buscomaravillas(self, value):
        ...
        for index, element in enumerate(self.elements):
            other_valuestr=str(element['other_value'])
            if (value in other_valuestr) and ("arquitectónica" in other_valuestr):
                conM=conM+1
            elif (value in other_valuestr) and ("Natural" in other_valuestr):
                conN=conN+1

        if conM>0 and conN>0:
            print(value ,"contiene maravillas de ambos tipos")

    def buscomaravillas222(self, value):
        conM=0
        conN=0
        for index, element in enumerate(self.elements):
            other_valuestr=str(element['other_value'])
        ...
        if conM>1:
            print(value ,"dispone de ",conM,"maravillas arquitectónica")
        elif conN>1:
            print(value ,"dispone de ",conN,"maravillas Naturales")
    ...
    def insert_vertice(self, value, other_value):
        nodo = {
        'value': value,
    ...
    def deep_showdevolviendo(self, origin):
        def __deep_show(graph, origin):
            pos_vertice = graph.search(origin)
            if pos_vertice is not None:
                if not graph.elements[pos_vertice]['visitado']:
